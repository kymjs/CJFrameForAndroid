 ...
 */
public class CJProxyActivity extends Activity implements I_LoadPlugin {

    private static final String TAG = CJProxyActivity.class.getSimpleName();
    private ActivityPlugin remotePlugin;

    @Override
    public ActivityPlugin loadPlugin(Activity ctx, String apkPath) {
        // 插件必须要确认有没有经过初始化，不然只是空壳
        remotePlugin = new ActivityPlugin(ctx, apkPath);
        return remotePlugin;
    }
    ...
    public ActivityPlugin loadPlugin(Activity ctx, String apkPath,
            String activityName) {
        ActivityPlugin plugin = loadPlugin(ctx, apkPath);
     ...
     */
    @Override
    public void fillPlugin(ActivityPlugin plugin) {
        if (plugin == null) {
        ...
        if (!apk.exists())
            throw new RuntimeException(apkPath);
     ...
     * @param plugin
     */
    private void fillPluginActivity(ActivityPlugin plugin) {
        try {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        Thread.setDefaultUncaughtExceptionHandler(new Thread.UncaughtExceptionHandler() {
            ...
            public void uncaughtException(Thread thread, final Throwable ex) {
                PluginException bug = new PluginException();
                bug.error = ex;
        ...
        if (pluginMessage != null) {
            pluginActivityName = pluginMessage.getString(
                    CJConfig.KEY_PLUGIN_ACT_NAME,
                    CJConfig.DEF_PLUGIN_CLASS_NAME);
            pluginDexPath = pluginMessage.getString(
                    CJConfig.KEY_PLUGIN_DEX_PATH, CJConfig.DEF_PLUGIN_DEX_PATH);
        } else {
            throw new RuntimeException("Please put the Plugin Path!");
        }
        if (pluginDexPath == CJConfig.DEF_PLUGIN_DEX_PATH) {
            throw new RuntimeException("Please put the Plugin Path!");
        }

        remotePlugin = loadPlugin(CJProxyActivity.this, pluginDexPath);

        if (pluginActivityName != CJConfig.DEF_PLUGIN_CLASS_NAME) {
    ...
    protected void onResume() {
        ...
        I_CJActivity caller = remotePlugin.getControl();
        if (caller != null) {
            caller.callOnResume();
